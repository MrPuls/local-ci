...
const engineIcon = computed(() =>
    engine.value && !engine.value.ready ? "cross" : "dot",
);
    ...
    <header class="panel" style="padding: 0.8rem 1rem" data-test-id="top-bar">
        ...
        >
            <div style="display: flex; align-items: baseline; gap: 0.6rem">
                <span
                    class="accent glow-strong"
                    style="font-size: 1.8rem; letter-spacing: 3px"
                    >LOCAL_CI</span
                >
                <span class="dim" v-if="version" data-test-id="version"
                    >V{{ version }}</span
                >
                <span
                    class="error"
                    v-else-if="online === false"
                    data-test-id="offline"
                    >OFFLINE</span
                >
            </div>
            ...
            <div style="display: flex; align-items: baseline; gap: 0.6rem">
                ...
                <span
                    :class="[statusClass, 'glow-strong']"
                    data-test-id="run-status-label"
            ...
            <div
                ...
                data-test-id="engine-status"
                :title="engineTitle"
            >
                <span class="dim">ENGINE:</span>
                <span :class="engineClass"
                    ><Icon :name="engineIcon" glow /> {{ engineLabel }}</span
                >
            </div>
